...
export const verifyEmail = async (req, res, next) => {
    ...
    const existingUser = await Auth.findOne({ email: decodedToken.email });
    if (!existingUser) {
        return next({
            cause: 404,
...
export const resetPassword = async (req, res, next) => {
    ...
    // Check if the user exists
    const existingUser = await Auth.findById(decodedToken.id);
    if (!existingUser) {
        return next({
            cause: 404,
            message: "المستخدم غير موجود",
...
export const getUserById = async (req, res, next) => {
    const { id } = req.params;
    const user = await Auth.findById(id).select("-password -__v");
    if (!user) {
        return next({
            cause: 404,
            message: "المستخدم غير موجود",
